...
function ApplySubscribe_jw() {
  ...
  function addBtnOnClick(e) {
    ...
    [a,c,t] = contentsSelect(contentsData, resultData, e)
          ...
          <div className={style.main}>
            <div className={style.mainLeft}>
              ...
              <div className={style.subscribeContentArea}>
                {contentsData.map((value, index) => {
                  return (
                    <div key={value.id} ref={canvasRef} className={style.subscribeContentBox}>
                      {onOff[index]?<><CanvasImage url={value.url}/></>:""}
                      <div className={style.contentIcon}>
                        <img src={value.url} alt="" />
                      </div>
                      <div className={onOff[index]?style.contentArea1:style.contentArea}>
                        <div className={style.content}>
                          <div className={onOff[index]?style.contentTitle1:style.contentTitle}>
                            {value.title} &gt;
                          </div>
                          <div className={onOff[index]?style.contentText1:style.contentText}>{value.text}</div>
                          <button
                            id={value.id}
            ...
            <div className={style.mainRight}>
              <div className={style.rightContentArea}>
                {isTrue ? (
                  <AddCart totalPrice={totalPrice} getKeyConvertJS={key} />
                ) : (
                  ""
                )}
