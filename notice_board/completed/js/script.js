...
function ScrollEvent() {
    ...
    document.addEventListener('scroll', () => {
        ...
        ////////////////////////      메인화면 스크롤 시 문구나 이미지가 나타나는 기능        /////////////
        function appearToScroll(progressBar) {

            let formular1 = 90 * (progressBar - 0.14) * (progressBar - 0.36),
                formular2 = 100 * (progressBar - 0.46) * (progressBar - 0.68),
                formular3 = 160 * (progressBar - 0.78) * (progressBar - 0.99);
            // console.log("percentage : " + progressBar)



            if (progressBar >= 0.0 && progressBar < 0.30) {
                $(`#main_content1`).css({
                    opacity: 1,
            ...
            else if (progressBar >= 0.3 && progressBar < 0.475) {
                $(`#main_content1`).css({
                    opacity: 0,
                    transition: "opacity 0.5s"
                })
            ...
            else if (progressBar >= 0.475 && progressBar < 0.65) {
                $(`#main_content1`).css({
                    opacity: 0,
                    transition: "opacity 0.5s"
                })
        ...
        //////////////////////////   index.html 하단의 시작 버튼을 누르면 처음으로 돌아가면서 게시판 링크 화면 보이는 기능 //////////////
        function clickMoveStart() {
            try {
                let clickMoveStartbtn = document.getElementById('clickMoveStartbtn');

                clickMoveStartbtn.onclick = () => {
                    ...
                    clickMoveStartbtn.style.display = "none";
        ...
        function progressBar(window_Y, scroll_Y) {
            ...
            return (window_Y / scroll_Y);
        };

        progressBar(window_Y, scroll_Y);

        appearToScroll(progressBar(window_Y, scroll_Y));
    ...
    function logInProcess(modal_logIn, logIn, logOut) {
        ...
        modal_logInButton.onclick = () => {
            ...
            let welcome = document.getElementById("welcome"),
                ...
                matchID = new RegExp("^[a-zA-Z0-9]{4,10}$"),
                matchPW = new RegExp("^(?=.*[0-9])(?=.*[a-zA-Z])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,15}$"),
                matchIDPW = [matchID, matchPW];

            if (matchIDPW[0].test(user[0]) && matchIDPW[1].test(user[1])) {
                logInToken = true;
                if (logInToken) {
                    welcome.innerHTML = `<p style="font-size:12px">반갑습니다<pre>     </pre>${user[0]}님</p><a href="" style="margin-right:15px;"><i style="font-size:20px; margin-left:15px" class="fa-regular fa-id-card"></i></a>`
                    $("#modal_aroundBlur").hide();
                    $("body").removeClass("hidden")
...
// 회원가입 등록할 때 아이디나 비밀번호가 조건에 맞는지, 중복된 ID인지 확인하는 기능
